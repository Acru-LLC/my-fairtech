...
import DocsService from "@/shared/services/docsService";
...
export default {
  ...
  methods: {
    ...
    getById(id) {
      DocsService.getById(id).then((rs) => {
        if (rs.data) {
          this.setConfg(rs);
        }
      });
    ...
    setConfg(rs) {
      setTimeout(() => {
        let config = {
          ...
          width: "100%",
          editorConfig: {
            callbackUrl: `${this.getRootApi()}/document/callback?type=track&fileName=${
              rs.data.title
            }.${rs.data.fileType}&userAddress=`,
            lang: 'ru',
          },
        };
