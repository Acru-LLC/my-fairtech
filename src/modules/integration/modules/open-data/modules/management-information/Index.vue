            ...
            <div class="d-inline-block">
              ...
              <div class="text-sm-end float-right">
                <download-excel
                    :data="json_data"
                    :fields="json_fields"
                    :header="$t('open_data.management_information.title')"
                    worksheet="My Worksheet"
                    :name="`${$t('open_data.management_information.title')}.xls`"
                >
                  <b-btn
                      @click="downloadExcel"
                      type="button"
              ...
              <b-btn
                  ...
                  :to="{name: 'CreateOpenDataManagementInformation'}"
              >
          ...
          >
            ...
            <template #cell(name)="data">
              <div class="d-flex justify-content-between">
                <multi-lang-cell lang="O'Z">{{ data.item.nameLt }}</multi-lang-cell>
                <multi-lang-cell lang="ЎЗ">{{ data.item.nameUz }}</multi-lang-cell>
                <multi-lang-cell lang="РУ">{{ data.item.nameRu }}</multi-lang-cell>
                <multi-lang-cell lang="EN">{{ data.item.nameEn }}</multi-lang-cell>
              </div>
            </template>
          ...
          <div class="row">
            <div class="col-md-12">
              ...
              <div class="form-group">
                <a :href="`/template/opendata/${$t('open_data.management_information.code')}.xlsx`" class="btn btn-info" target="_blank" download>{{ $t('actions.download_template') }}</a>
              </div>
            </div>
            <div class="col-md-12">
...
import MultiLangCell from '../../components/MultiLangCell'
...
export default {
  ...
  data() {
    return {
      ...
      json_fields: {
        [this.$t('column.fio')]: this.getName({
          nameUz: 'fullNameUz',
          nameRu: 'fullNameRu',
          nameEn: 'fullNameEn',
          nameLt: 'fullNameLt',
        }),
        [this.$t('column.position')]: this.getName({
          nameUz: 'positionUz',
          nameRu: 'positionRu',
          nameEn: 'positionEn',
          nameLt: 'positionLt',
        }),
        [this.$t('column.reception_days')]: this.getName({
          nameUz: 'receptionDaysUz',
          nameRu: 'receptionDaysRu',
          nameEn: 'receptionDaysEn',
          nameLt: 'receptionDaysLt',
        }),
        [this.$t('column.time')]: this.getName({
          nameUz: 'timeUz',
          nameRu: 'timeRu',
          nameEn: 'timeEn',
          nameLt: 'timeLt',
      ...
      tableFields: [
        ...
        {
          label: this.$t('column.fio'), key: this.getName({
            nameUz: 'fullNameUz',
            nameRu: 'fullNameRu',
            nameEn: 'fullNameEn',
            nameLt: 'fullNameLt',
        ...
        {
          label: this.$t('column.position'), key: this.getName({
            nameUz: 'positionUz',
            nameRu: 'positionRu',
            nameEn: 'positionEn',
            nameLt: 'positionLt',
        ...
        {
          label: this.$t('column.reception_days'), key: this.getName({
            nameUz: 'receptionDaysUz',
            nameRu: 'receptionDaysRu',
            nameEn: 'receptionDaysEn',
            nameLt: 'receptionDaysLt',
        ...
        {
          label: this.$t('column.time'), key: this.getName({
            nameUz: 'timeUz',
            nameRu: 'timeRu',
            nameEn: 'timeEn',
            nameLt: 'timeLt',
  ...
  methods: {
    ...
    promptOpenDataToken(){
      this.promptSwal({
        ...
        if (isConfirmed) {
          openDataService.createToken(this.$t('open_data.management_information.code'), value)
        }
      })
    },
    fetchSendOpenData() {
    ...
    fetchTableItems() {
      ...
      if (this.$i18n.locale === 'uz') {
        this.var_default_search_payload.sortBy.push('fullNameLt')
      } else if (this.$i18n.locale === 'ru') {
        this.var_default_search_payload.sortBy.push('fullNameRu')
      } else if (this.$i18n.locale === 'uzCyrillic') {
        this.var_default_search_payload.sortBy.push('fullNameUz')
      }
      crudAndListsService
    ...
    editItem(id) {
      this.$router.push({name: 'UpdateOpenDataManagementInformation', params: {id: id}})
    },
    viewItem(id) {
      this.$router.push({name: 'ViewOpenDataManagementInformation', params: {id: id}})
    },
    deleteItem(id) {
