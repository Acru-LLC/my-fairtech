            ...
            <div class="d-inline-block">
              ...
              <div class="text-sm-end float-right">
                <download-excel
                    :data="json_data"
                    :fields="json_fields"
                    :header="$t('open_data.competition_law_reestr.title')"
                    worksheet="My Worksheet"
                    :name="`${$t('open_data.competition_law_reestr.title')}.xls`"
                >
                  <b-btn
                      @click="downloadExcel"
                      type="button"
              ...
              <b-btn
                  ...
                  :to="{name: 'CreateOpenDataCompetitionLawReestr'}"
              >
          ...
          <div class="row">
            <div class="col-md-12">
              ...
              <div class="form-group">
                <a :href="`/template/opendata/${$t('open_data.competition_law_reestr.code')}.xlsx`" class="btn btn-info" target="_blank" download>{{ $t('actions.download_template') }}</a>
              </div>
            </div>
            <div class="col-md-12">
...
export default {
  ...
  data() {
    return {
      ...
      json_fields: {
        [this.$t('open_data.competition_law_reestr.enterpriseName', 'uzCyrillic')]: 'enterpriseNameUz',
        [this.$t('open_data.competition_law_reestr.enterpriseName', 'ru')]: 'enterpriseNameRu',
        [this.$t('open_data.competition_law_reestr.enterpriseName', 'uz')]: 'enterpriseNameLt',
        [this.$t('open_data.competition_law_reestr.enterpriseName', 'en')]: 'enterpriseNameEn',
        [this.$t('open_data.competition_law_reestr.number')]: 'number',
        [this.$t('open_data.competition_law_reestr.date')]: 'date',
        [this.$t('open_data.competition_law_reestr.document', 'uzCyrillic')]: 'documentUz',
        [this.$t('open_data.competition_law_reestr.document', 'ru')]: 'documentRu',
        [this.$t('open_data.competition_law_reestr.document', 'uz')]: 'documentLt',
        [this.$t('open_data.competition_law_reestr.document', 'en')]: 'documentEn',
      },
      json_data: [],
      json_meta: [
        [
      ...
      tableFields: [
        ...
        {
          label: this.$t('open_data.competition_law_reestr.enterpriseName'), key: this.getName({
            nameUz: 'enterpriseNameUz',
            nameRu: 'enterpriseNameRu',
            nameLt: 'enterpriseNameLt',
          }),
        },
        {
          label: this.$t('open_data.competition_law_reestr.number'), key: 'number',
        },
        {
          label: this.$t('open_data.competition_law_reestr.date'), key: 'date',
        },
        {
          label: this.$t('open_data.competition_law_reestr.document'), key: this.getName({
            nameUz: 'documentUz',
            nameRu: 'documentRu',
            nameLt: 'documentLt',
          }),
  ...
  methods: {
    ...
    promptOpenDataToken(){
      this.promptSwal({
        ...
        if (isConfirmed) {
          openDataService.createToken(this.$t('open_data.competition_law_reestr.code'), value)
        }
      })
    },
    fetchSendOpenData() {
    ...
    fetchTableItems() {
      ...
      if (this.$i18n.locale === 'uz') {
        this.var_default_search_payload.sortBy.push('enterpriseNameLt')
      } else if (this.$i18n.locale === 'ru') {
        this.var_default_search_payload.sortBy.push('enterpriseNameRu')
      } else if (this.$i18n.locale === 'uzCyrillic') {
        this.var_default_search_payload.sortBy.push('enterpriseNameUz')
      }
      crudAndListsService
    ...
    editItem(id) {
      this.$router.push({name: 'UpdateOpenDataCompetitionLawReestr', params: {id: id}})
    },
    viewItem(id) {
      this.$router.push({name: 'ViewOpenDataCompetitionLawReestr', params: {id: id}})
    },
    deleteItem(id) {
