            ...
            <div class="d-inline-block">
              ...
              <div class="text-sm-end float-right">
                <download-excel
                    :data="json_data"
                    :fields="json_fields"
                    :header="$t('open_data.vacancy.title')"
                    worksheet="My Worksheet"
                    :name="`${$t('open_data.vacancy.title')}.xls`"
                >
                  <b-btn
                      @click="downloadExcel"
                      type="button"
          ...
          <div class="row">
            <div class="col-md-12">
              ...
              <div class="form-group">
                <a :href="`/template/opendata/${$t('open_data.vacancy.code')}.xlsx`" class="btn btn-info" target="_blank" download>{{ $t('actions.download_template') }}</a>
              </div>
            </div>
            <div class="col-md-12">
...
export default {
  ...
  data() {
    return {
      ...
      json_fields: {
        [this.$t('open_data.vacancy.job_vacancy')]: this.getName({
          nameUz: 'jobVacancyUz',
          nameRu: 'jobVacancyRu',
          nameLt: 'jobVacancyLt',
        }),
        [this.$t('open_data.vacancy.vacancy_number')]: 'number'
      },
      json_data: [],
      json_meta: [
        [
      ...
      tableFields: [
        ...
        {
          label: this.$t('open_data.vacancy.job_vacancy'), key: this.getName({
            nameUz: 'jobVacancyUz',
            nameRu: 'jobVacancyRu',
            nameLt: 'jobVacancyLt',
          }),
        },
        {label: this.$t('open_data.vacancy.vacancy_number'), key: 'number'},
        {
  ...
  methods: {
    ...
    promptOpenDataToken(){
      this.promptSwal({
        ...
        if (isConfirmed) {
          openDataService.createToken(this.$t('open_data.vacancy.code'), value)
        }
      })
    },
    fetchSendOpenData() {
    ...
    fetchTableItems() {
      ...
      if (this.$i18n.locale === 'uz') {
        this.var_default_search_payload.sortBy.push('jobVacancyLt')
      } else if (this.$i18n.locale === 'ru') {
        this.var_default_search_payload.sortBy.push('jobVacancyRu')
      } else if (this.$i18n.locale === 'uzCyrillic') {
        this.var_default_search_payload.sortBy.push('jobVacancyUz')
      }
      crudAndListsService
    ...
    editItem(id) {
      this.$router.push({name: 'UpdateOpenDataVacancy', params: {id: id}})
    },
    viewItem(id) {
      this.$router.push({name: 'ViewOpenDataVacancy', params: {id: id}})
    },
    deleteItem(id) {
