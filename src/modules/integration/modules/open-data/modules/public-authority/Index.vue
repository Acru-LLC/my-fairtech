    ...
    <b-col cols="12">
      <b-col md="12" class="text-center">
        <div class="h4 mb-4 d-inline-block">
          {{ $t('open_data.public_authority.code') }} - {{ $t('open_data.public_authority.title') }}
        </div>
      </b-col>
      <b-card>
        <b-card-body>
            ...
            <div class="d-inline-block">
              ...
              <div class="text-sm-end float-right">
                <download-excel
                    :data="json_data"
                    :fields="json_fields"
                    :header="$t('open_data.public_authority.title')"
                    worksheet="My Worksheet"
                    :name="`${$t('open_data.public_authority.title')}.xls`"
                >
                  <b-btn
                      @click="downloadExcel"
                      type="button"
              ...
              <b-btn
                  ...
                  :to="{name: 'CreateOpenDataPublicAuthority'}"
              >
          ...
          <div class="row">
            <div class="col-md-12">
              ...
              <div class="form-group">
                <a :href="`/template/opendata/${$t('open_data.public_authority.code')}.xlsx`" class="btn btn-info" target="_blank" download>{{ $t('actions.download_template') }}</a>
              </div>
            </div>
            <div class="col-md-12">
...
export default {
  ...
  data() {
    return {
      ...
      json_fields: {
        [this.$t('open_data.non_compliant_brand.organizationName', 'uz')]: 'organizationNameLt',
        [this.$t('open_data.non_compliant_brand.organizationName', 'uzCyrillic')]: 'organizationNameUz',
        [this.$t('open_data.non_compliant_brand.organizationName', 'ru')]: 'organizationNameRu',
        [this.$t('open_data.non_compliant_brand.organizationName', 'en')]: 'organizationNameEn',
        [this.$t('open_data.non_compliant_brand.address', 'uz')]: 'addressLt',
        [this.$t('open_data.non_compliant_brand.address', 'uzCyrillic')]: 'addressUz',
        [this.$t('open_data.non_compliant_brand.address', 'ru')]: 'addressRu',
        [this.$t('open_data.non_compliant_brand.address', 'en')]: 'addressEn',
        [this.$t('open_data.non_compliant_brand.latitude')]: 'latitude',
        [this.$t('open_data.non_compliant_brand.longitude')]: 'longitude',
        [this.$t('open_data.non_compliant_brand.addressLocation')]: 'addressLocation',
        [this.$t('open_data.non_compliant_brand.phone')]: 'phone',
      },
      json_data: [],
      json_meta: [
        [
      ...
      tableFields: [
        ...
        {
          label: this.$t('open_data.public_authority.organizationName'), key: this.getName({
        ...
        {
          label: this.$t('open_data.public_authority.address'), key: this.getName({
            nameUz: 'addressUz',
            nameRu: 'addressRu',
            nameLt: 'addressLt',
          }),
        ...
        {
          label: this.$t('open_data.public_authority.latitude'), key: 'latitude',
        },
        {
          label: this.$t('open_data.public_authority.longitude'), key: 'longitude',
        },
        {
          label: this.$t('open_data.public_authority.addressLocation'), key: 'addressLocation',
        },
        {
          label: this.$t('open_data.public_authority.phone'), key: 'phone',
        },
        {
  ...
  methods: {
    ...
    promptOpenDataToken(){
      this.promptSwal({
        ...
        if (isConfirmed) {
          openDataService.createToken(this.$t('open_data.public_authority.code'), value)
        }
      })
    },
    fetchSendOpenData() {
    ...
    editItem(id) {
      this.$router.push({name: 'UpdateOpenDataPublicAuthority', params: {id: id}})
    },
    viewItem(id) {
      this.$router.push({name: 'ViewOpenDataPublicAuthority', params: {id: id}})
    },
    deleteItem(id) {
