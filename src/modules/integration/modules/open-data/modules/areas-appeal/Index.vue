    ...
    <b-col cols="12">
      <b-col md="12" class="text-center">
        <div class="h4 mb-4 d-inline-block">
          {{ $t('open_data.areas_appeal.code') }} - {{ $t('open_data.areas_appeal.title') }}
        </div>
      </b-col>
      <b-card>
        <b-card-body>
            ...
            <div class="d-inline-block">
              ...
              <div class="text-sm-end float-right">
                <download-excel
                    :data="json_data"
                    :fields="json_fields"
                    :header="$t('open_data.areas_appeal.title')"
                    worksheet="My Worksheet"
                    :name="`${$t('open_data.areas_appeal.title')}.xls`"
                >
                  <b-btn
                      @click="downloadExcel"
                      type="button"
              ...
              <b-btn
                  ...
                  :to="{name: 'CreateOpenDataAreasAppeal'}"
              >
          ...
          <div class="row">
            <div class="col-md-12">
              ...
              <div class="form-group">
                <a :href="`/template/opendata/${$t('open_data.areas_appeal.code')}.xlsx`" class="btn btn-info" target="_blank" download>{{ $t('actions.download_template') }}</a>
              </div>
            </div>
            <div class="col-md-12">
...
<script>
const MAIN_API_URL = 'open-data/areas-appeal'
...
export default {
  ...
  data() {
    return {
      ...
      json_fields: {
        [this.$t('open_data.areas_appeal.home')]: 'home',
        [this.$t('open_data.areas_appeal.sale')]: 'sale',
        [this.$t('open_data.areas_appeal.communication')]: 'communication',
        [this.$t('open_data.areas_appeal.advertising')]: 'advertising',
      },
      json_data: [],
      json_meta: [
        [
      ...
      tableFields: [
        ...
        {label: this.$t('open_data.areas_appeal.home'), key: 'home'},
        {label: this.$t('open_data.areas_appeal.sale'), key: 'sale'},
        {label: this.$t('open_data.areas_appeal.communication'), key: 'communication'},
        {label: this.$t('open_data.areas_appeal.advertising'), key: 'advertising'},
        {
  ...
  methods: {
    ...
    promptOpenDataToken(){
      this.promptSwal({
        ...
        if (isConfirmed) {
          openDataService.createToken(this.$t('open_data.areas_appeal.code'), value)
        }
      })
    },
    fetchSendOpenData() {
    ...
    fetchTableItems() {
      ...
      this.var_default_search_payload.sortBy.push('home')
      crudAndListsService
    ...
    editItem(id) {
      this.$router.push({name: 'UpdateOpenDataAreasAppeal', params: {id: id}})
    },
    viewItem(id) {
      this.$router.push({name: 'ViewOpenDataAreasAppeal', params: {id: id}})
    },
    deleteItem(id) {
