    ...
    <b-col cols="12">
      <b-col md="12" class="text-center">
        <div class="h4 mb-4 d-inline-block">
          {{ $t('open_data.appeals_consideration.code') }} - {{ $t('open_data.appeals_consideration.title') }}
        </div>
      </b-col>
      <b-card>
        <b-card-body>
            ...
            <div class="d-inline-block">
              ...
              <div class="text-sm-end float-right">
                <download-excel
                    :data="json_data"
                    :fields="json_fields"
                    :header="$t('open_data.appeals_consideration.title')"
                    worksheet="My Worksheet"
                    :name="`${$t('open_data.appeals_consideration.title')}.xls`"
                >
                  <b-btn
                      @click="downloadExcel"
                      type="button"
              ...
              <b-btn
                  ...
                  :to="{name: 'CreateOpenDataAppealsConsideration'}"
              >
          ...
          <div class="row">
            <div class="col-md-12">
              ...
              <div class="form-group">
                <a :href="`/template/opendata/${$t('open_data.appeals_consideration.code')}.xlsx`" class="btn btn-info" target="_blank" download>{{ $t('actions.download_template') }}</a>
              </div>
            </div>
            <div class="col-md-12">
...
<script>
const MAIN_API_URL = 'open-data/appeals-consideration'
...
export default {
  ...
  data() {
    return {
      ...
      json_fields: {
        [this.$t('open_data.appeals_consideration.name')]: this.getName({
          nameUz: 'nameUz',
          nameRu: 'nameRu',
          nameLt: 'nameLt',
        }),
        [this.$t('open_data.appeals_consideration.number')]: 'number',
        [this.$t('open_data.appeals_consideration.verbally')]: 'verbally',
        [this.$t('open_data.appeals_consideration.written')]: 'written',
        [this.$t('open_data.appeals_consideration.electron')]: 'electron',
      },
      json_data: [],
      json_meta: [
        [
      ...
      tableFields: [
        ...
        {
          label: this.$t('open_data.appeals_consideration.name'), key: this.getName({
            nameUz: 'nameUz',
            nameRu: 'nameRu',
            nameLt: 'nameLt',
          }),
        },
        {label: this.$t('open_data.appeals_consideration.number'), key: 'number'},
        {label: this.$t('open_data.appeals_consideration.verbally'), key: 'verbally'},
        {label: this.$t('open_data.appeals_consideration.written'), key: 'written'},
        {label: this.$t('open_data.appeals_consideration.electron'), key: 'electron'},
        {
  ...
  methods: {
    ...
    promptOpenDataToken(){
      this.promptSwal({
        ...
        if (isConfirmed) {
          openDataService.createToken(this.$t('open_data.appeals_consideration.code'), value)
        }
      })
    },
    fetchSendOpenData() {
    ...
    editItem(id) {
      this.$router.push({name: 'UpdateOpenDataAppealsConsideration', params: {id: id}})
    },
    viewItem(id) {
      this.$router.push({name: 'ViewOpenDataAppealsConsideration', params: {id: id}})
    },
    deleteItem(id) {
