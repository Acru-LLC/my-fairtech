            ...
            <div class="d-inline-block">
              ...
              <div class="text-sm-end float-right">
                <download-excel
                    :data="json_data"
                    :fields="json_fields"
                    :header="$t('open_data.explanation_and_warning.title')"
                    worksheet="My Worksheet"
                    :name="`${$t('open_data.explanation_and_warning.title')}.xls`"
                >
                  <b-btn
                      @click="downloadExcel"
                      type="button"
              ...
              <b-btn
                  ...
                  :to="{name: 'CreateOpenDataExplanationAndWarning'}"
              >
          ...
          <div class="row">
            <div class="col-md-12">
              ...
              <div class="form-group">
                <a :href="`/template/opendata/${$t('open_data.explanation_and_warning.code')}.xlsx`" class="btn btn-info" target="_blank" download>{{ $t('actions.download_template') }}</a>
              </div>
            </div>
            <div class="col-md-12">
...
export default {
  ...
  data() {
    return {
      ...
      json_fields: {
        [this.$t('open_data.explanation_and_warning.areaName')]: this.getName({
          nameUz: 'areaNameUz',
          nameRu: 'areaNameRu',
          nameLt: 'areaNameLt',
        }),
        [this.$t('open_data.explanation_and_warning.name')]: 'name',
        [this.$t('open_data.explanation_and_warning.mjtk')]: 'mjtk',
        [this.$t('open_data.explanation_and_warning.fine')]: 'fine',
        [this.$t('open_data.explanation_and_warning.sum')]: 'sum',
        [this.$t('open_data.explanation_and_warning.rejectedFine')]: 'rejectedFine',
      },
      json_data: [],
      json_meta: [
        [
      ...
      tableFields: [
        ...
        {
          label: this.$t('open_data.explanation_and_warning.areaName'), key: this.getName({
            nameUz: 'areaNameUz',
            nameRu: 'areaNameRu',
            nameLt: 'areaNameLt',
          }),
        },
        {label: this.$t('open_data.explanation_and_warning.name'), key: 'name'},
        {label: this.$t('open_data.explanation_and_warning.mjtk'), key: 'mjtk'},
        {label: this.$t('open_data.explanation_and_warning.fine'), key: 'fine'},
        {label: this.$t('open_data.explanation_and_warning.sum'), key: 'sum'},
        {label: this.$t('open_data.explanation_and_warning.rejectedFine'), key: 'rejectedFine'},
        {
  ...
  methods: {
    ...
    promptOpenDataToken(){
      this.promptSwal({
        ...
        if (isConfirmed) {
          openDataService.createToken(this.$t('open_data.explanation_and_warning.code'), value)
        }
      })
    },
    fetchSendOpenData() {
    ...
    editItem(id) {
      this.$router.push({name: 'UpdateOpenDataExplanationAndWarning', params: {id: id}})
    },
    viewItem(id) {
      this.$router.push({name: 'ViewOpenDataExplanationAndWarning', params: {id: id}})
    },
    deleteItem(id) {
