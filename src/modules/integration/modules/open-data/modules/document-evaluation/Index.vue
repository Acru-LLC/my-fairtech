            ...
            <div class="d-inline-block">
              ...
              <div class="text-sm-end float-right">
                <download-excel
                    :data="json_data"
                    :fields="json_fields"
                    :header="$t('open_data.document_evaluation.title')"
                    worksheet="My Worksheet"
                    :name="`${$t('open_data.document_evaluation.title')}.xls`"
                >
                  <b-btn
                      @click="downloadExcel"
                      type="button"
              ...
              <b-btn
                  ...
                  :to="{name: 'CreateOpenDataDocumentEvaluation'}"
              >
          ...
          <div class="row">
            <div class="col-md-12">
              ...
              <div class="form-group">
                <a :href="`/template/opendata/${$t('open_data.document_evaluation.code')}.xlsx`" class="btn btn-info" target="_blank" download>{{ $t('actions.download_template') }}</a>
              </div>
            </div>
            <div class="col-md-12">
...
export default {
  ...
  data() {
    return {
      ...
      json_fields: {
        [this.$t('open_data.document_evaluation.regionalOfficesName')]: this.getName({
          nameUz: 'regionalOfficesNameUz',
          nameRu: 'regionalOfficesNameRu',
          nameLt: 'regionalOfficesNameLt',
        }),
        [this.$t('open_data.document_evaluation.total')]: 'total',
        [this.$t('open_data.document_evaluation.noObjection')]: 'noObjection',
        [this.$t('open_data.document_evaluation.antiCompetitiveNumber')]: 'antiCompetitiveNumber',
        [this.$t('open_data.document_evaluation.antiCompetitivePercent')]: 'antiCompetitivePercent',
        [this.$t('open_data.document_evaluation.deficiency')]: 'deficiency',
      },
      json_data: [],
      json_meta: [
        [
      ...
      tableFields: [
        ...
        {
          label: this.$t('open_data.document_evaluation.regionalOfficesName'), key: this.getName({
            nameUz: 'regionalOfficesNameUz',
            nameRu: 'regionalOfficesNameRu',
            nameLt: 'regionalOfficesNameLt',
          }),
        },
        {label: this.$t('open_data.document_evaluation.total'), key: 'total'},
        {label: this.$t('open_data.document_evaluation.noObjection'), key: 'noObjection'},
        {label: this.$t('open_data.document_evaluation.antiCompetitiveNumber'), key: 'antiCompetitiveNumber'},
        {label: this.$t('open_data.document_evaluation.antiCompetitivePercent'), key: 'antiCompetitivePercent'},
        {label: this.$t('open_data.document_evaluation.deficiency'), key: 'deficiency'},
        {
  ...
  methods: {
    ...
    promptOpenDataToken(){
      this.promptSwal({
        ...
        if (isConfirmed) {
          openDataService.createToken(this.$t('open_data.document_evaluation.code'), value)
        }
      })
    },
    fetchSendOpenData() {
    ...
    fetchTableItems() {
      ...
      if (this.$i18n.locale === 'uz') {
        this.var_default_search_payload.sortBy.push('regionalOfficesNameLt')
      } else if (this.$i18n.locale === 'ru') {
        this.var_default_search_payload.sortBy.push('regionalOfficesNameRu')
      } else if (this.$i18n.locale === 'uzCyrillic') {
        this.var_default_search_payload.sortBy.push('regionalOfficesNameUz')
      }
      crudAndListsService
    ...
    editItem(id) {
      this.$router.push({name: 'UpdateOpenDataDocumentEvaluation', params: {id: id}})
    },
    viewItem(id) {
      this.$router.push({name: 'ViewOpenDataDocumentEvaluation', params: {id: id}})
    },
    deleteItem(id) {
