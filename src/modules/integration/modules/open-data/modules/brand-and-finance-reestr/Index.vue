    ...
    <b-col cols="12">
      <b-col md="12" class="text-center">
        <div class="h4 mb-4 d-inline-block">
          {{ $t('open_data.brand_and_finance_reestr.code') }} - {{ $t('open_data.brand_and_finance_reestr.title') }}
        </div>
      </b-col>
      <b-card>
        <b-card-body>
            ...
            <div class="d-inline-block">
              ...
              <div class="text-sm-end float-right">
                <download-excel
                    :data="json_data"
                    :fields="json_fields"
                    :header="$t('open_data.brand_and_finance_reestr.title')"
                    worksheet="My Worksheet"
                    :name="`${$t('open_data.brand_and_finance_reestr.title')}.xls`"
                >
                  <b-btn
                      @click="downloadExcel"
                      type="button"
...
export default {
  data() {
    return {
      ...
      json_fields: {
        [this.$t('open_data.brand_and_finance_reestr.subjectName', 'uzCyrillic')]: 'subjectNameUz',
        [this.$t('open_data.brand_and_finance_reestr.subjectName', 'ru')]: 'subjectNameRu',
        [this.$t('open_data.brand_and_finance_reestr.subjectName', 'uz')]: 'subjectNameLt',
        [this.$t('open_data.brand_and_finance_reestr.subjectName', 'en')]: 'subjectNameEn',
        [this.$t('open_data.brand_and_finance_reestr.stir')]: 'stir',
        [this.$t('open_data.brand_and_finance_reestr.regions', 'uzCyrillic')]: 'regionUz',
        [this.$t('open_data.brand_and_finance_reestr.regions', 'ru')]: 'regionRu',
        [this.$t('open_data.brand_and_finance_reestr.regions', 'uz')]: 'regionLt',
        [this.$t('open_data.brand_and_finance_reestr.regions', 'en')]: 'regionEn',
      },
      json_data: [],
      json_meta: [
        [
      ...
      tableFields: [
        ...
        {
          label: this.$t('open_data.brand_and_finance_reestr.subjectName'), key: this.getName({
        ...
        {
          label: this.$t('open_data.brand_and_finance_reestr.stir'), key: 'stir',
        },
        {
          label: this.$t('open_data.brand_and_finance_reestr.regions'), key: this.getName({
            nameUz: 'regionUz',
            nameRu: 'regionRu',
            nameLt: 'regionLt',
          }),
  ...
  methods: {
    ...
    promptOpenDataToken(){
      this.promptSwal({
        ...
        if (isConfirmed) {
          openDataService.createToken(this.$t('open_data.brand_and_finance_reestr.code'), value)
        }
      })
    },
    fetchSendOpenData() {
    ...
    editItem(id) {
      this.$router.push({name: 'UpdateOpenDataBrandAndFinanceReestr', params: {id: id}})
    },
    viewItem(id) {
      this.$router.push({name: 'ViewOpenDataBrandAndFinanceReestr', params: {id: id}})
    },
    deleteItem(id) {
