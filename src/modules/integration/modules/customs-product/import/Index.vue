    ...
    <b-col cols="12">
      <b-col md="12" class="text-center">
        <div class="h4 mb-4 d-inline-block">
          {{ $t('submodules.integration.customs_product.title') }} - {{ $t('submodules.integration.customs_product.import') }}
        </div>
      </b-col>
      <b-card>
        <b-card-body>
          <b-row class="mb-2">
            ...
            <div class="ml-1 d-inline-block" v-if="$can('download', 'to excel')">
              <div class="text-sm-end float-right">
          ...
          >
            <template #top-row>
              <b-th/>
              <b-th variant="secondary" class="pt-0">
                <b-row style="transform: scale(0.8);margin: -7px 0px -7px -38px;width: 135%;">
                  <b-col sm="6" class="pr-1 text-center">
                    {{ $t('column.from') }}
                  </b-col>
                  <b-col sm="6" class="pl-1 text-center">
                    {{ $t('column.to') }}
                  </b-col>
 ...
 */
export default {
  components: { BaseDatePickerWithValidation },
  data() {
    return {
  ...
  methods: {
    async downloadExcel() {
      this.json_data = this.tableItems.map((item) => {
        return {
          ...item,
    ...
    searchListWithKeyword(mainUrl, payload, requestParams = {}, withLoader = false) {
      ...
      myPayload.page -= 1
      let url = mainUrl + '/list-search/import';
      if (requestParams) {
    ...
    viewItem(id) {
      this.$router.push({ name: 'ViewIntegrationCustomsProductImport', params: { id: id } })
    },
  },
  created() {
    this.fetchTableItems()
