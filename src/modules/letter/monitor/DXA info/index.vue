<template>
  <div class="card mb-0">
    ...
    <div class="pt-4 pl-5">
      ...
      <b-btn
          v-if="['DESCENDED', 'RETURN_FOR_COMPLETE', 'RETURN_AFTER_NO_PERMIT_REQUIRED', 'RETURN_AFTER_NOT_BELONG', 'RETURN_FROM_REGION', 'RETURN_AFTER_REJECTED'].includes(applicationStatus)"
          class="mr-2  float-right btn-xx"
          @click="$emit('acceptApplication','SEND_REGION')"
          >
        <img
            :src="require('@/assets/doc/5.png')"
            alt="DOC"
            height="20"
        />
        {{ $t("column.accept") }}
      ...
      <b-btn
          v-show="['DESCENDED', 'RETURN_FOR_COMPLETE', 'RETURN_AFTER_NO_PERMIT_REQUIRED', 'RETURN_AFTER_NOT_BELONG', 'RETURN_FROM_REGION', 'RETURN_AFTER_REJECTED'].includes(applicationStatus)"
          variant="light"
          class="mr-2 btn btn-outline-danger float-right"
          @click="$emit('noPermitRequired', applicationId)"
      >
        {{ $t("submodules.dxa.no_permit_required") }}
      </b-btn>
      <b-btn
          v-show="['DESCENDED', 'RETURN_FOR_COMPLETE', 'RETURN_AFTER_NO_PERMIT_REQUIRED', 'RETURN_AFTER_NOT_BELONG', 'RETURN_FROM_REGION', 'RETURN_AFTER_REJECTED'].includes(applicationStatus)"
    ...
    <div class="card-body p-5">
      <b-overlay :opacity="0.1" :show="loadingById" >
        <div class="card">
          <table class="table m-0">
            <tr>
              ...
              <td class="text-right">
                ...
                <span style="font-size: 12px" class="text-left" v-if="visaInfoById.applicationType === 'FOR_SALE'" >
                  {{ $t("submodules.dxa.dxa_application_1") }}
                </span>
                <span style="font-size: 12px" class="text-left" v-else-if="visaInfoById.applicationType === 'FOR_ADD'" >
        ...
        <div class="card">
          <div class="timeline">
            ...
            <div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
              <div class="tabs">
                <div class="tab" v-for="(tabsItem, index) in selectedApplicationTabsStep" :key="index">
                  <div class="panel panel-default">
                    <div class="panel-heading" role="tab" id="heading1">
                      <div class="icon">
                        <i class="glyphicon glyphicon-one-fine-dot">
                          <span class="sr-only">Expand/Collapse </span>
                        </i>
                      </div>
                      <input type="checkbox" :id="`rd-${index}`" name="rd" checked="checked">
                          ...
                          <div v-if="tabsItem.fields && tabsItem.fields.length > 0" class="card">
                            <table class="table m-0">
                              <tr v-for="(tabsByStepId, index2) in tabsItem.fields" :key="index2">
                                ...
                                <td class="text-right font-weight-normal" v-if="tabsByStepId.type === 'FILE'" style="width: 200px">
                                  <i class="fa fa-eye"
                                     v-b-tooltip.hover
    ...
    <b-modal
        :title="$t('submodules.dxa.not_complete') "
        id="widthMM"
        v-model="isCommit"
        size="lg"
        :no-close-on-backdrop="true"
...
export default {
  ...
  methods: {
    ...
    async sendReview() {
      ...
      await LetterService.applicationReviewNOT_COMPLETE(obj, this.applicationId)
          .then((res) => {
...
.btn-xx {
  background-color: #928ef8 !important;
  cursor: pointer;
  color: #fff;
  border: 2px solid #1f0df8;
  top: -5px !important;
}
