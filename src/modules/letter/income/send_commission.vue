...
import {cnfDoc, showMsgSuccess, showMsgError} from "@/helper";
...
export default {
  ...
  methods: {
    ...
    async signSuccess(data) {
      ...
      Service.onlyDocSignSendToRais(payload, this.currentDoc.id,
          this.x,
          this.y,
    ...
    getByIdLetter() {
      Service.getByIdSendToRais(this.$route.params.id)
          .then((rs) => {
                ...
                this.loaderPdf = true;
                if (rs.data.projectType == 'FROM_DXA') {
    ...
    save() {
      if (this.qrCodePage != null && this.imgUrl && this.src) {
        if (this.currentDoc.letterType == 'REFERENCE') {
