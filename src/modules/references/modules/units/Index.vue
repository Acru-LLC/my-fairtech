  ...
  <div class="row">
    <div class="col-12">
      <div class="col-md-12 text-center">
        <div class="h4 mb-4 d-inline-block">{{ $t('column.units') }}</div>
      </div>
      <div class="card">
        <div class="card-body">
          <div class="row mb-2">
            ...
            <template #cell(shortName)="data">
              <div class="d-flex justify-content-between">
                <p
                    class="mb-0 d-flex align-items-center"
                    style="flex-basis: 0; flex-grow: 1; gap: .3rem;"
                ><span class="badge bg-primary">ЎЗ</span> : <span> {{ data.item.shortNameUz }}</span></p>
                <p
                    class="mb-0 d-flex align-items-center"
                    style="flex-basis: 0; flex-grow: 1; gap: .3rem;"
                ><span class="badge bg-primary">O'Z</span> : <span> {{ data.item.shortNameLt }}</span></p>
                <p
                    class="mb-0 d-flex align-items-center"
                    style="flex-basis: 0; flex-grow: 1; gap: .3rem;"
                ><span class="badge bg-primary">РУ</span> : <span> {{ data.item.shortNameRu }}</span></p>
              </div>
            </template>
...
const MAIN_API_URL = 'directory/units'
  ...
  data() {
    return {
      json_fields: {
        ...
        "Name Lt": "nameLt",
        "Name Ru": "nameRu",
        "Қисқа номи Uz": "shortNameUz",
        "Қисқа номи Lt": "shortNameLt",
        "Қисқа номи Ru": "shortNameRu",
      },
      json_data: [],
      json_meta: [
        [
      ...
      tableFields: [
        ...
        {label: this.$t('column.short_name'), key: "shortName"},
        {
  ...
  methods: {
    downloadExcel() {
      ...
      this.tableItems.forEach(res => {
        ...
        obj.shortNameUz = res.shortNameUz
        obj.shortNameLt = res.shortNameLt
        obj.shortNameRu = res.shortNameRu
    ...
    editItem(id) {
      this.$router.push({name: 'UpdateUnit', params: {id: id}})
    },
    deleteItem(id) {
