    ...
    <div class="col-12">
      <div class="col-md-12 text-center">
        <div class="h4 mb-4 d-inline-block">
          {{ $t('submodules.integration.price_stock.product_name_title') }}
        </div>
        <router-link class="btn btn-success float-right" :to="{name: 'ReferencesPriceStockProductNameCreate'}">
          {{$t('actions.create')}}
        </router-link>
      </div>
      <div class="card">
          ...
          >
            ...
            <template #cell(name)="data">
              <div class="d-flex justify-content-between">
                ...
                ><span class="badge bg-primary">ЎЗ</span> : <span> {{ data.item.productNameUz }}</span></p>
                <p
                    class="mb-0 d-flex align-items-center"
                    style="flex-basis: 0; flex-grow: 1; gap: .3rem;"
                ><span class="badge bg-primary">O'Z</span> : <span> {{ data.item.productNameLt }}</span></p>
                <p
                    class="mb-0 d-flex align-items-center"
                    style="flex-basis: 0; flex-grow: 1; gap: .3rem;"
                ><span class="badge bg-primary">РУ</span> : <span> {{ data.item.productNameRu }}</span></p>
              </div>
            </template>
...
const REF_NAME = 'price/stock/product-name'
...
export default {
  ...
  data() {
    return {
      ...
      title: this.$t('submodules.integration.price_stock.product_name_title'),
      activeDep: {},
      tableItems: [],
      totalItems: 0,
      tableFields: [
        ...
        {
          label: this.$t('column.connected_product'),
          key: "spProductName"
        },
        {
          label: this.$t('submodules.integration.price_stock.product_name'),
          key: "name"
        },
        {
  ...
  methods: {
    ...
    editItem(id) {
      this.$router.push({ name: 'ReferencesPriceStockProductNameUpdate', params: { id: id } })
    },
  },
  /* CREATED */
  created() {
