    ...
    <div class="row">
        <div class="col-12">
          <div class="col-md-12 text-center">
            <div class="h4 mb-4 d-inline-block">{{ $t('submodules.ad_volume_types.title') }}</div>
          </div>
            <div class="card">
                <div class="card-body">
                    <div class="row mb-2">
              ...
              <span>{{ $t('column.select.text2') }}</span> -->
                    ...
                    <div class="text-sm-end">
                      <download-excel
                          :data="json_data"
                          :fields="json_fields"
                          :header="$t('submodules.ad_volume_types.title')"
                          worksheet="My Worksheet"
                          :name="`${$t('submodules.ad_volume_types.title')}.xls`"
                      >
                        <b-btn
                            @click="downloadExcel"
                            type="button"
                        ...
                        >
                          <i class="mdi mdi-microsoft-excel me-1"></i> {{ $t('actions.download') }}
                                ...
                                <b-btn
                                    ...
                                    :to="{name: 'CreateAdvertisementVolumeType'}"
                                >
                    ...
                    >
                        ...
                        <template #cell(value)="data">
                            <b>{{ data.item.minBorder }}</b> {{ `${$t('column.square_m')} ${$t('column.from')}` }} <span v-if="!data.item.maxNotLimited"><b>{{ data.item.maxBorder }}</b> {{ `${$t('column.square_m')} ${$t('column.to')}` }}</span> <span v-else> юқори</span>
                        </template>
    ...
    data () {
        return {
            ...
            tableFields: [
                ...
                { label: this.$t('column.value_square_m'), key: "value" },
                {
    ...
    methods: {
        ...
        fetchTableItems () {
            ...
            crudAndListsService
                ...
                .then((res) => {
                    this.tableItems = res.data ? res.data.list : [];
                })
                .catch(e => {
                    this.tableItems = [];
                })
        ...
        editItem (id) {
            this.$router.push({ name: 'UpdateAdvertisementVolumeType', params: { id: id } })
        },
        deleteItem (id) {
