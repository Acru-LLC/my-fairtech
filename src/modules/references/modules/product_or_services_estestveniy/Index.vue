<template>
  <div class="row">
    <div class="col-12">
      <div class="card">
        <div class="ml-5 mt-3 mb-0" style="text-align: center">
          <h4> {{ $t('submodules.product_or_services_estestveniy.title') }}</h4>
        </div>
        <div class="card-body">
          <div class="row mb-2">
            <div class="col-sm-8">
                ...
                <b-row>
                  ...
                  <b-col cols="5">
                    <b-btn
                        ...
                        :to="{name: 'CreateProductOrServiceEstestveniy', params: {cStatusId: contractorStatuses[activeTabIndex].code.toLowerCase()}}"
                    >
          ...
          <!--                    getName({-->
          <!--                      nameRu: cStatus.nameRu,-->
          <!--                      nameUz: cStatus.nameUz,-->
          <!--                      nameLt: cStatus.nameLt-->
          <!--                    })-->
          <!--                  }}</span>-->
          <!--              </template>-->
          <!-- Table data -->
          ...
          >
            ...
            <template #cell(index)="data">
              {{ util_paginate(data.index, paginations[activeTabIndex].page, paginations[activeTabIndex].itemsPerPage) }}
            </template>
            ...
            <template #cell(product_or_service_type)="data">
              <p>
                {{
                  getName({
                    nameRu: data.item.directoryProductOrServiceTypeNameRu,
                    nameLt: data.item.directoryProductOrServiceTypeNameLt,
                    nameUz: data.item.directoryProductOrServiceTypeNameUz,
                  })
                }}
              </p>
            </template>
          ...
          <!-- end table -->
          <b-pagination
              :key="`tab-pagination-${activeTabIndex}`"
              v-model="paginations[activeTabIndex].page"
              @input="fetchTableItems(activeTabIndex)"
              :total-rows="paginations[activeTabIndex].totalItems"
              :per-page="paginations[activeTabIndex].itemsPerPage"
...
export default {
  ...
  data() {
    return {
      ...
      tableFields: [
        ...
        {label: this.$t('submodules.product_or_service_types.title'), key: "product_or_service_type"},
        {label: this.$t('column.code'), key: "code"},
        {label: this.$t('column.skp_code'), key: "skpcode"},
        {label: this.$t('column.tnved_code'), key: "tnvedcode"},
  ...
  methods: {
    ...
    selectList($event) {
      if ($event == 'all') {
        this.paginations[this.activeTabIndex].itemsPerPage = this.totalItems
      } else {
        this.paginations[this.activeTabIndex].itemsPerPage = $event
      }
    ...
    fetchTableItems(tabIndex) {
      ...
      if (this.contractorStatuses.length) {
        this.paginations[this.activeTabIndex].keyword = this.searchKeyword
        ...
        crudAndListsService
            .searchListWithKeyword(MAIN_API_URL, this.paginations[this.activeTabIndex], this.contractorStatuses[this.activeTabIndex].code.toLowerCase(), true)
            .then((res) => {
              this.tableItems = res.data.list;
              // this.totalItems = res.data.total;
              this.paginations[this.activeTabIndex].totalItems = res.data.total
            })
            .catch(e => {
              this.tableItems = [];
              // this.totalItems = 0;
    ...
    editItem(id) {
      this.$router.push({
        ...
        params: {id: id, cStatusId: this.contractorStatuses[this.activeTabIndex].code.toLowerCase()}
      })
    },
    deleteItem(id) {
  ...
  async created() {
    ...
    await helperService.getRefByCode('contractor_status', true)
        .then(res => {
          ...
          this.contractorStatuses.forEach(el => {
            let tempPagination = Object.assign({}, this.var_default_search_payload)
            tempPagination.totalItems = 0
            this.paginations.push(tempPagination)
          })
