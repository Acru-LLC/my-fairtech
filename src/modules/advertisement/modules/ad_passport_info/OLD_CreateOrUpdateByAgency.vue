<template>
  ...
  >
      ...
      <b-row class="mb-3">
        ...
        >
          <BaseInputWithValidation
              ...
              :label="$t('column.ad_construction_width')"
          />
        </b-col>
      </b-row>
        ...
        >
          <b-col cols="4">
            <BaseDatePickerWithValidation
                not-required
                custom-styles="grid-template-columns: 57% 43%;"
                v-model="editingItem1.constructDate"
      ...
      <b-row class="mb-3">
        ...
        >
          <b-row class="mb-3">
            <BaseMultiselectWithValidation
                rules="required"
                class="required"
                v-model="editingItem1.directoryAdvertisementObjectStateId"
                :options="adObjectState.map(el => el.id)"
          ...
          <b-row class="mb-3">
            <BaseInputWithValidation
                not-required
                disabled
                v-model="editingItem1.monthlyPayment"
        ...
        >
          <BaseTextareaWithValidation
              not-required
              v-model="editingItem1.description"
              max-rows="8"
    ...
    <b-modal
        v-model="contractorCreateSidebar"
        ...
        no-close-on-esc
    >
      <CreateFormContractor :custom-is-mode-create="true"></CreateFormContractor>
    </b-modal>
  </base-create-or-update-wrapper>
</template>
<script>
...
export default {
  ...
  * COMPUTED */
  computed: {
    isModeCreate() {
      return this.$route.name === 'CreateAdPassportInfoByAgency'
    },
    computedObserver() {
  ...
  * METHODS */
  methods: {
    ...
    customObjectState(opt) {
      let selected = this.adObjectState.find(e => e.id == (opt.id ? opt.id : opt));
      if (selected) {
        return `${
            this.getName({
              nameRu: selected.nameRu,
    ...
    async fetchContractors() {
      ...
      await crudAndListsService.searchListWithKeyword('contractor', this.var_default_search_payload)
          .then(res => {
            this.contractors.push(...res.data.list)
            this.hasNextPageContractors = res.data.total / this.var_default_search_payload.itemsPerPage > this.var_default_search_payload.page
            this.var_default_search_payload.page += 1
          })
          .catch(e => {
            console.log(e)
  ...
  async created() {
    ...
    if (this.isModeCreate) {
      await crudAndListsService.getEmpty(MAIN_API_URL)
          .then(res => {
            this.editingItem = res.data.contractorAdvertisingConstructionForFirstAgentDto
            this.editingItem1 = res.data.contractorAdvertisingConstructionForSecondAgentDto
          })
          .catch(e => {
            console.log(e)
          })
    ...
    crudAndListsService
        ...
        .then((res) => {
          this.adObjectState = res.data.list;
        })
        .catch(e => {
          console.log(e)
        })
