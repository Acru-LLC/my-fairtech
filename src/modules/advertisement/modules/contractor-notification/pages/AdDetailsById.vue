            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex align-items-center justify-content-between mb-2">
                        ...
                        <h4 class="m-0 mb-1"><strong>{{ $t('column.passport_number')}}: {{ currentAdDetails.passportNumber}}</strong></h4>
                    </div>
                        ...
                        <div class="ad-details-wrapper">
                            <table class="ad-details">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ currentAdDetails.contractorAddressRegionNameUz }}, {{ currentAdDetails.contractorAddressDistrictNameUz }} {{ currentAdDetails.contractorAdditionalAddress ? ' ,' + currentAdDetails.contractorAdditionalAddress : ''}}</td>
                                    </tr>
                                    <tr>
                            ...
                            >
                              <b-row class="pr-md-0 pr-lg-2">
                                ...
                                <b-col>
                                    <p class="p-0 m-0"><strong>{{ $t('column.created_by')}}:</strong> {{ currentAdDetails.createdByEmployeeFullName ? (currentAdDetails.createdByEmployeeFullName + ' ' + (currentAdDetails.createdByDirectoryPositionNameUz ? ('(' + currentAdDetails.createdByDirectoryPositionNameUz + ', ' + $t('auth.username') + ' - ' + currentAdDetails.createdByUserName + ')') : '')) : $t('auth.username') + ' - ' + currentAdDetails.createdByUserName}} <br><i>{{ currentAdDetails.createdDate }}</i></p>
                                    <p class="p-0 m-0"><strong>{{ $t('column.last_modified_by')}}:</strong> {{ currentAdDetails.updatedByEmployeeFullName ? (currentAdDetails.updatedByEmployeeFullName + ' ' + (currentAdDetails.updatedByDirectoryPositionNameUz ? ('(' + currentAdDetails.updatedByDirectoryPositionNameUz + ', ' + $t('auth.username') + ' - ' + currentAdDetails.updatedByUserName + ')') : '')) : $t('auth.username') + ' - ' + currentAdDetails.updatedByUserName}} <br><i>{{ currentAdDetails.updatedDate }}</i></p>
                                ...
                                <b-col>
                                  <div class="float-right">
                                    <VueQriously :value="qrCodeHeader + '/details/' +currentAdDetails.id" :size="100"/>
                                  </div>
                                </b-col>
...
export default {
    ...
    * METHODS */
    async created () {
        this.getAdDetails(this.$route.params.adId)
    },
    /*
    * CREATED */
    computed: {
