  ...
  <div class="row">
    <div class="col-12">
      <div class="col-md-12 text-center">
        <div class="h4 mb-4 d-inline-block">{{ $t('submodules.contractor_notification.all_notifications_list') }}</div>
      </div>
      <div class="card">
        <div class="card-body">
          <div v-if="!currentUserIsOuter" class="row mb-2">
            <div class="col-sm-6">
          ...
          <div v-if="!currentUserIsOuter" class="row mb-2">
            <div class="col-sm-12">
              <b-row>
                <b-col cols="6" v-if="!currentUserIsOuter">
                  <!-- CONTRAGENTS -->
                  <BaseMultiselectWithValidation
                      not-required
                      multiple
          ...
          >
            ...
            <template #cell(actions)="data">
              <div class="general-table__actions d-flex justify-content-center">
                ...
                <b-btn
                    v-if="$can('list', 'advertisement notification by contractorAdvertisingConstructionId')"
                    v-b-tooltip.hover title="Хабарномалар"
                    variant="link"
                    class="text-decoration-none p-0"
                    style="font-size: 1.2rem; margin-right: 1rem;"
                >
                  <i v-if="data.item.contractorAdvertisingConstructionForSecondAgentDto.isPresent"
                     @click="fetchNotificationTableItems(data.toggleDetails, data.item.contractorAdvertisingConstructionForSecondAgentDto.id)"
                     class="bx bxs-bell bx-tada" style="color: green"></i>
                  <i v-else
                     @click="fetchNotificationTableItems(data.toggleDetails, data.item.contractorAdvertisingConstructionForSecondAgentDto.id)"
                     class="bx bxs-bell"></i>
                </b-btn>
                ...
                <b-btn
                    v-if="$can('create', 'advertisement notification')"
                    v-b-tooltip.hover title="Хабарноманома қўшиш"
                    variant="link"
                    class="text-decoration-none p-0"
                    style="font-size: 1.2rem; margin-right: 1rem;"
                >
                  <i
                      @click="createItem(data.item.contractorAdvertisingConstructionForSecondAgentDto.id)"
            ...
            <template #row-details="row">
                ...
                <div class="col-5 search-box me-4 mb-2 d-inline-block">
                  <div class="position-relative">
                    <input
                        ...
                        type="text"
                        class="form-control"
                        @input="fetchNotificationTableItems(null, row.item.contractorAdvertisingConstructionForSecondAgentDto.id)"
                        :placeholder="$t('column.search')"
                    />
                    <i class="bx bx-search-alt search-icon"></i>
                  </div>
              ...
              <b-table
                  :items="row.item.contractorAdvertisingConstructionForSecondAgentDto.notifications ? row.item.contractorAdvertisingConstructionForSecondAgentDto.notifications : []"
                ...
                <!-- STATUS -->
                <template #cell(statusNames)="innerData">
                  <b-badge v-if="innerData.item.statusCode === 'ACCEPTED'" variant="success">{{
                      getName({
                        nameRu: innerData.item.statusNameRu,
                        nameLt: innerData.item.statusNameLt,
                        nameUz: innerData.item.statusNameUz,
                      })
                    }}
                  </b-badge>
                  <b-badge v-else-if="innerData.item.statusCode === 'REJECTED'" variant="danger">{{
                      getName({
                        nameRu: innerData.item.statusNameRu,
                        nameLt: innerData.item.statusNameLt,
                        nameUz: innerData.item.statusNameUz,
                  ...
                  <b-badge v-else-if="innerData.item.statusCode === 'VIEWED'" variant="warning">{{
                      getName({
                        nameRu: innerData.item.statusNameRu,
                        nameLt: innerData.item.statusNameLt,
                        nameUz: innerData.item.statusNameUz,
...
export default {
  ...
  methods: {
    ...
    fetchTableItems() {
      ...
      } else if (this.currentUserIsOuter) {
        this.loadingTableItems = true
        this.var_default_search_payload.keyword = this.searchKeyword
        this.var_default_search_payload.regionId = this.filterPayload.regionId
        this.var_default_search_payload.districtIds = this.filterPayload.districtIds
    ...
    fetchNotificationTableItems(toggleDetails, id) {
      if (toggleDetails) {
        toggleDetails()
      }
      this.loadingExtendTableItems = true
      this.var_default_search_payload.keyword = this.searchNotificationKeyword
      ...
      crudAndListsService
          ...
          .then((res) => {
            let foundAdIndex = this.tableItems.findIndex(el => el.contractorAdvertisingConstructionForSecondAgentDto.id == id)
            if (foundAdIndex > -1) {
              this.$set(this.tableItems[foundAdIndex].contractorAdvertisingConstructionForSecondAgentDto, 'notifications', res.data.list)
            }
          })
          .catch(e => {
  ...
  async created() {
    ...
    // GET CONTRACTORS
    if (!this.currentUserIsOuter && this.$can('list','contractor')) {
