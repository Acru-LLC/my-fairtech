...
import GeneratedColumnAndRows from "../components/viewReport";
...
export default {
  ...
  methods: {
    save() {
      ...
      Service.fillDocReport(thirdTrIdData, dec(this.$route.query.reportId))
        .then((rs) => {
          if (rs.data) {
            this.successSaved();
          }
    ...
    getById() {
      setTimeout(() => {
        this.$refs.viewRef.getById(dec(this.$route.params.id), dec(this.$route.query.reportId));
      }, 300);
    },
  },
};
