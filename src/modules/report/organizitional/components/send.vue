<template>
  <div style="min-height: 200px" class="border-color-custom">
    ...
    <!-- KELISHISH -->
    <div class="card" v-if="selectedSignature.id">
      <div class="card-header bg-white d-flex align-items-center">
            ...
            <tr>
              <td style="width: 50px">
                <img
                    ...
                    :src="`${publicPath}/${selectedSignature.uploadPath}`"
              ...
              <td>
                ...
                <p class="m-0 text-muted">
                  {{
                    getName({
                      nameLt: selectedSignature.departmentNameLt,
                      nameRu: selectedSignature.departmentNameRu,
                      nameUz: selectedSignature.departmentNameUz,
                    })
                  }}
                </p>
                <p class="m-0 text-muted">
                  {{
                    getName({
                      nameLt: selectedSignature.directoryPositionNameLt,
                      nameRu: selectedSignature.directoryPositionNameRu,
                      nameUz: selectedSignature.directoryPositionNameUz,
                    })
                  }}
                </p>
              </td>
    ...
    <div class="card" v-if="selectedReview && selectedReview.id">
      <div class="card-header bg-white d-flex align-items-center">
      ...
      <div>
        <div class="table-responsive m-0">
          <table class="table table-centered m-0">
            <tbody v-if="selectedReview && selectedReview.id">
            <tr>
              <td>
                <h5 class="font-size-14 m-0">
...
export default {
  ...
  methods: {
    ...
    asyncValue_2(data) {
      let res = this.setAsync2ValueRecursive(this.$refs.partRef_2.contactList, data);
      if (!data || !data.id || !res) {
        this.selectedReview = {}
      }
    },
    setAsync2ValueRecursive(contactList, data){
      let l = contactList.length
      for (let i = 0; i < l; i++) {
        if (contactList[i].id === data.id) {
          return this.selectedReview = contactList[i];
        }
        if (contactList[i].children) {
          if (this.setAsync2ValueRecursive(contactList[i].children, data)) {
