...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      Service.listDocReportGathered(this.params, this.tabStatus)
          .then((rs) => {
...
<template>
  <div>
    <div class="col-md-12 text-center">
      <div class="h4 mb-4 d-inline-block">{{ $t('submodules.reports.report_all') }}</div>
    </div>
    <b-modal
        scrollable
    ...
    >
      <EditReportModal :key="cmpKey+'modal_edit_key'" ref="editReportModalRef"/>

      <template v-slot:modal-footer>
        <b-button variant="secondary" @click="editReportModal = false">
          {{ $t("actions.close") }}
        </b-button
        >
