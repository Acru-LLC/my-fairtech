...
export default {
  ...
  methods: {
    ...
    isVerticalChange(data) {
      let foundIndex = this.selectedColumn.findIndex(el => el.id == data.id)
      if (foundIndex > -1) {
        this.$set(this.selectedColumn[foundIndex], 'isVertical', data.isVertical)
      }
      // this.$emit( "setColumns", this.selectedColumn );
    },
    reset(array = []) {
    ...
    getClass(data) {
      ...
      if (found) {
        this.list.find(el => el.id == data.id).isVertical = this.selectedColumn.find((e) => e.id == data.id).isVertical
      }
      return found;
    },
  },
      ...
      <b-overlay :opacity="0.1" :show="loading" rounded="sm">
        <table class="table table-centered table-custom-bordered-column">
          ...
          <template v-for="(data, index) in list">
            <Tr
                @pushData="pushData"
                @isVerticalChange="isVerticalChange"
                :sidebar="sidebar"
                :index="index"
                :key="data.id + 'Column'"
