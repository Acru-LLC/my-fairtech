<template>
  ...
  >
    <ValidationObserver ref="observer" v-slot="{}">
      ...
      <b-row v-if="isModeCreate" class="mb-3" >
        <b-col sm="12" md="6" >
          <BaseInputWithValidation
              name="password"
              with-input-append
      ...
      <b-row v-if="!isModeCreate && wantToChangePassword" class="mb-3" >
        <b-col sm="12" md="6" >
          <BaseInputWithValidation
              name="newPassword"
              with-input-append
...
export default {
  ...
  * METHODS */
  methods: {
    ...
    fetchEmployees(depId) {
      this.employees = []
      helperService
          .searchEmployeesByDep( this.var_default_search_payload, depId, true)
          .then((res) => {
    ...
    checkNoUsername(searchQuery = "", fromOnBlur = false) {
      ...
      if (!searchQuery || this.checkUsernameLoading || (!this.isModeCreate && this.editingItem.username == this.dbUsername)) return;
          ...
          if (this.editingItem.id) {
            crudAndListsService.update(MAIN_API_URL, this.editingItem).then(res => {
              ...
              if (this.wantToChangePassword) {
                ...
                helperService.updateUserPasswordByAdmin(this.passwordPayload, true)
                    .then(res => {
  ...
  * CREATED */
  async created() {
    ...
    if (this.isModeCreate) {
      ...
      // GET EMPLOYEES
      await helperService.getEmployeeList('', true)
          .then(res => {
    ...
    // GET ROLES
    await crudAndListsService.searchList('role', this.var_default_search_payload, null, true)
        .then(res => {
