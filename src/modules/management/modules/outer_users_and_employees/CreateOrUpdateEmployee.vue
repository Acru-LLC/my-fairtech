...
<!--                    ></BaseTreeselectWithValidation>-->
            ...
            <b-row class="mb-3">
                ...
                >
                  <BaseInputWithValidation
                      not-required
                      v-model="editingItem.positionName"
                    ...
                    >
                        <template #first>
                            <b-form-select-option
                                :value="null"
                                disabled
                            >{{ $t('column.gender') }}</b-form-select-option>
                          <b-form-select-option v-for="(gender, index) in genders" :key="`${gender}-${index}`" :value="gender.id">{{
                              getName({
            ...
            <b-row class="mb-3">
                ...
                >
                    <BaseInputWithValidation
                        rules="required|integer|min:7|max:7"
                        class="required"
...
export default {
    ...
    * COMPUTED */
    computed: {
        isModeCreate () {
            return this.$route.name === 'CreateOuterEmployee'
        },
        computedObserver () {
    ...
    * METHODS */
    methods: {
        forceInputUppercase (e) {
            ...
            this.editingItem.passportSeries = e.target.value
        ...
        getIndividualInfoByInn () {
            if (this.computedObserver.fields.INN.valid)
                helperService.validateInnFromApi(this.editingItem.inn)
                    .then(res => {
                        if (res.data) {
                            helperService.getIndividualInfoByInn(this.editingItem.inn)
                                .then(res => {
    ...
    * CREATED */
    async created () {
        ...
        // GET GENDERS
        await helperService.getRefByCodeNew('gender')
            .then(res => {
        ...
        // GET POSITIONS
        /* await crudAndListsService.searchList('directory/position', this.var_default_search_payload)
            .then(res => {
