        ...
        <b-row class="mb-3">
            ...
            >
                <BaseMultiselectWithValidation
                    rules="required"
                    class="required"
                    v-model="editingItem.directoryServiceTypeId"
                    :options="serviceTypes.map(el => el.id)"
                    :label="$t('submodules.service_types.title')"
                    :custom-label="customLabelServiceType"
                    :placeholder="''"
...
<script>
const MAIN_API_URL = 'directory/services'
/*
...
export default {
    ...
    * COMPUTED */
    computed: {
        isModeCreate () {
            return this.customIsModeCreate ? this.customIsModeCreate : this.$route.name === 'CreateService'
        },
        computedObserver () {
    ...
    * METHODS */
    methods: {
        customLabelServiceType (opt) {
            let selected = this.serviceTypes.find(e => e.id == (opt.id ? opt.id : opt));
            if (selected) {
                return `${this.getName({
    ...
    * CREATED */
    async created () {
        ...
        // GET SERVICE_TYPES
        crudAndListsService
            .searchList('directory/service-type', this.var_default_search_payload)
            .then((res) => {
