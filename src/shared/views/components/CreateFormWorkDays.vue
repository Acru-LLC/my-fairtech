    ...
    <b-row class="mb-3">
      ...
      >
        <BaseInputWithValidation
            @input="($event) => {
              editingItem.nameUz = toCyrill($event)
              editingItem.nameLt = toLatin($event)
            }"
            rules="required"
            class="required"
...
<script>
const MAIN_API_URL = 'directory/work-days';
/*
 * YOU MUST SEND {{ MAIN_API_URL }} TO CRUD_SERVICE */
import crudAndListsService from "@/shared/services/crud_and_list.service"

export default {
  name: "CreateFormWorkDays",
  props: {
    customIsModeCreate: {
      type: Boolean,
      default: false
  ...
  * COMPUTED */
  computed: {
    isModeCreate() {
      return this.customIsModeCreate ? this.customIsModeCreate : this.$route.name === 'CreateWorkDays'
    },
    computedObserver() {
