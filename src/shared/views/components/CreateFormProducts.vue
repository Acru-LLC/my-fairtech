        ...
        <b-row class="mb-3">
            ...
            >
                <BaseMultiselectWithValidation
                    rules="required"
                    class="required"
                    v-model="editingItem.directoryProductTypeId"
                    :options="productTypes.map(el => el.id)"
                    :label="$t('submodules.product_types.title')"
...
<script>
const MAIN_API_URL = 'directory/products'
/*
...
export default {
    ...
    * COMPUTED */
    computed: {
        isModeCreate () {
            return this.customIsModeCreate ? this.customIsModeCreate : this.$route.name === 'CreateProduct'
        },
        computedObserver () {
    ...
    * CREATED */
    async created () {
        ...
        // GET PRODUCT_TYPES
        crudAndListsService
            .searchList('directory/product-type', this.var_default_search_payload)
            .then((res) => {
