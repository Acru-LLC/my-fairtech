    ...
    >
        <b-row class="mb-3">
            ...
            >
                <BaseMultiselectWithValidation
                    ...
                    :options="productTypes.map(el => el.id)"
                    :label="$t('submodules.product_or_service_types.title')"
        ...
        <b-row class="mb-3">
            ...
            >
                <BaseInputWithValidation
                    not-required
                    v-model="editingItem.skpcode"
                    :label="$t('column.skp_code')"
                    :placeholder="$t('column.skp_code')"
                />
            </b-col>
        </b-row>
        ...
        <b-row class="mb-3">
            ...
            >
                <BaseInputWithValidation
                    not-required
                    v-model="editingItem.tnvedcode"
                    :label="$t('column.tnved_code')"
                    :placeholder="$t('column.tnved_code')"
                />
            </b-col>
        </b-row>
...
export default {
    ...
    * COMPUTED */
    computed: {
        isModeCreate () {
            return this.customIsModeCreate ? this.customIsModeCreate : this.$route.name === 'CreateProductOrServiceDaminiriushiy'
        },
        computedObserver () {
    ...
    * METHODS */
    methods: {
        customLabelProductType (opt) {
            let selected = this.productTypes.find(e => e.id == (opt.id ? opt.id : opt));
            if (selected) {
                return `${this.getName({
    ...
    * CREATED */
    async created () {
        ...
        crudAndListsService
            .searchList('directory/product-or-service-types', this.var_default_search_payload, this.$route.params.cStatusId)
            .then((res) => {
